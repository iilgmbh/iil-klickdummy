...
def _spec_with_personas_plural() -> dict:
    return {
        "spec_id": "repo:demo", "spec_version": "0.1", "spec_date": "2026-06-01",
        "adr": {"local": "repo:ADR-001", "conforms_to": "platform:ADR-211"},
        "class": "mock",
        "personas": {"buerger": {"label": "Bürger", "rolle": "buerger",
                                 "description": "Antragsteller", "halbschicht": "buerger"}},
        "screens": [
            {"id": "antrag", "title": "Antrag stellen", "personas": ["buerger"],
             "purpose": "Antrag", "off_ramp_status": "static", "parity_acceptance": []},
            {"id": "status", "title": "Status ansehen", "personas": ["buerger"],
             "purpose": "Status", "off_ramp_status": "static", "parity_acceptance": []},
        ],
    }
...
def test_should_generate_uc_skeletons_from_plural_personas():
    ...
    records = [{"kind": "spec", "repo": "iil-klickdummy", "kd": "demo",
...
def _write_spec(path, spec_id):
    ...
    path.write_text(yaml.dump({"spec_id": spec_id, "spec_version": "0.1",
