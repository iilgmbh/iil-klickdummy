...
def _legacy_repo(base: pathlib.Path, name: str, n_hits: int) -> None:
    ...
    lines = "\n".join(f"- Zeile {i}: alter mock-prototyp Begriff" for i in range(n_hits))
...
def test_inventory_should_scan_all_repos_and_note_truncation(tmp_path, capsys):
    ...
    rc = inventory.main(["--base", str(tmp_path), "--repos", "repo-a,repo-b", "--strict"])
...
def test_off_ramp_status_explicit_overall_wins():
    spec = {"off_ramp": {"status_overall": "parity-green"},
...
def test_off_ramp_status_uniform_screens_aggregate():
    spec = {"off_ramp": {}, "screens": [
        {"off_ramp_status": "removed"}, {"off_ramp_status": "removed"}]}
    assert discovery_push._off_ramp_status(spec) == "removed"


def test_off_ramp_status_mixed_screens_yield_transition():
    spec = {"off_ramp": {}, "screens": [
        {"off_ramp_status": "static"}, {"off_ramp_status": "removed"}]}
...
def test_widget_should_have_reste_fixes():
    ...
    assert "window.confirm" in js, "GitHub-Fallback muss nachfragen, nicht auto-downloaden"
...
def test_cross_repo_stories_should_remap_kd_index(tmp_path):
    ...
    stories_dir.mkdir()
    (stories_dir / "s1.yaml").write_text(textwrap.dedent("""
        id: s1
        title: Beta-Story
        steps:
          - kd: beta
