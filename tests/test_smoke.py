...
def test_check_i2_strict_mode():
    ...
    assert check_i2.LEGACY == {}, (
        "Strict-Mode: LEGACY must be empty per ADR-211 Rev 12/13"
    )
...
def test_should_genesor_sync_canonical_source_be_importable_with_correct_interface(
    ...
    spec = importlib.util.spec_from_file_location(
        "klickdummy_sync_canonical", module_file
    )
    ...
    (tmp_path / "klickdummy" / "demo-kd" / "screens-spec.yaml").write_text(
        "title: Demo"
    )
    mod.ROOT = tmp_path
    ...
    (tmp_path / "docs" / "01-architektur" / "mockups" / "buerger-klickdummy").mkdir(
        parents=True
    )
    (
        tmp_path
...
def test_v11_browser_template_present():
    tmpl = (
        files("iil_klickdummy.snippets.browser")
        .joinpath("browser.html.tmpl")
        .read_text()
    )
...
def test_v11_registry_render_browser_html(tmp_path):
    ...
    fake = [
        registry.KlickdummyMeta(
            name="demo",
            path="klickdummy/demo/screens-spec.yaml",
            shell_path="klickdummy/demo/shell.html",
            spec_id="repo:klickdummy-spec-demo",
            spec_version="0.1",
            klickdummy_class="mock",
            title="Demo",
            adr_local="repo:ADR-100",
            sister_of=[],
        )
    ]
...
def test_v12_sync_entry_schema(tmp_path):
    ...
    fake = registry.KlickdummyMeta(
        ...
        title="Demo",
        adr_local="repo:ADR-100",
        sister_of=["other:ADR-099"],
    )
    entry = s.klickdummy_entry(
        fake, org="iilgmbh", repo="test-repo", repo_root=tmp_path
    )
...
def test_v13_cross_repo_render(tmp_path):
    ...
    fake = registry.KlickdummyMeta(
        name="x",
        path="klickdummy/x/screens-spec.yaml",
        shell_path="klickdummy/x/shell.html",
        spec_id="iilgmbh:klickdummy-spec-x",
        spec_version="0.1",
        klickdummy_class="mock",
        title="X",
        adr_local="iilgmbh:ADR-100",
        sister_of=[],
    )
...
def test_v17_trace_strip_declared_fields():
    ...
    assert (
        "vergabe-pruefen" in out and "los-verwalten" in out
    )  # echte UC-Namen (in <details>)
...
def test_v17_trace_strip_missing_fields_evidence_discipline():
    ...
    out = lineage.build_trace_strip(
        {"id": "leer", "title": "Leer"}, "mock", "default", {}
    )
...
def test_v17_trace_strip_renders_into_screen_section(tmp_path):
    ...
    spec = yaml.safe_load(
        ...
        .joinpath("screens-spec-template.yaml")
        .read_text()
    )
    record = {
        "data": spec,
...
def test_v116_discover_stories_resolves_steps(tmp_path):
    ...
    stories_dir.mkdir()
    (stories_dir / "test-journey.yaml").write_text(
        yaml.dump(
            {
...
def test_v119_validate_stories_clean(tmp_path):
    ...
    sdir = tmp_path / "klickdummy" / "stories"
    sdir.mkdir()
    (sdir / "j.yaml").write_text(
        yaml.dump(
            {
                "id": "test:story-journey",
                "title": "Journey",
                "steps": [
                    {"kd": "recherche", "label": "1"},
                    {"kd": "angebot", "label": "2"},
                ],
            }
        )
...
def test_v120_flow_dangling_is_error(tmp_path):
    ...
    sd = tmp_path / "klickdummy" / "kd"
    sd.mkdir(parents=True)
    (sd / "screens-spec.yaml").write_text(
        yaml.dump(
            {
                "spec_id": "t:klickdummy-spec-kd",
                "title": "KD",
                "class": "mock",
                "screens": [
...
def test_v117_write_stories_manifest(tmp_path):
    ...
    kd_a = registry.KlickdummyMeta(
        name="angebot",
        path="klickdummy/angebot/screens-spec.yaml",
...
def test_v122_local_entity_examples_override_synth():
    ...
    assert html.count("<tr><td>") == 2, (
        "Body-Zeilenzahl = Anzahl examples (nicht n_rows)"
    )
