...
def group_providers_by_contract(
...
def emit_mermaid(
    specs: list[tuple[str, Path, dict]], contracts: dict[str, Path]
) -> str:
    ...
    for cid, members in provider_groups.items():
        ...
        lines.append("")
        lines.append(
            f'    subgraph {sg_id} ["📋 Provider-Familie · {html.escape(short)}"]'
        )
    ...
    for kd_name, _path, data in specs:
        ...
        for screen in data.get("screens", []) or []:
            screen_ckl = (
                screen.get("cross_klickdummy_link")
    ...
    lines.append(
        "    classDef rootNode fill:#cef,stroke:#06c,stroke-width:2px,color:#000"
    )
    lines.append(
        "    classDef hybridNode fill:#fec,stroke:#c80,stroke-width:2px,color:#000"
    )
    lines.append(
        "    classDef contractNode fill:#fde,stroke:#c0c,stroke-width:2px,color:#000"
    )
    lines.append(
        "    classDef familyGroup fill:#f5f5f5,stroke:#888,stroke-dasharray:4 4,color:#444"
    )
...
def emit_screen_lineage(spec_data: dict) -> str:
    ...
    lines.append(
        "    classDef extNode fill:#fce7f3,stroke:#9f1239,stroke-dasharray:5 5,color:#000"
    )
    return "\n".join(lines)
