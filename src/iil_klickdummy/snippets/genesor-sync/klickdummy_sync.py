...
Heimat (Rev 13 §Distribution):
  iil-klickdummy/src/iil_klickdummy/snippets/genesor-sync/
"""
...
def find_specs() -> list[tuple[str, Path]]:
    ...
    for p in sorted(ROOT.glob("docs/01-architektur/mockups/*-klickdummy/screens-spec.yaml")):
...
def render_body(kd_name: str, spec_path: Path, data: dict) -> str:
    ...
    rel_md = "\n".join(rel_lines) if rel_lines else "_standalone — keine Cross-KD-Beziehungen_"
...
---
_Dieses Issue wird automatisch durch [`klickdummy-sync.yml`](.github/workflows/klickdummy-sync.yml) aus der Spec aktualisiert. Pipeline-Status, Zuweisungen, Kommentare bleiben in GitHub — sie überleben den nächsten Sync._
"""
...
def upsert_issue(kd_name: str, spec_path: Path, data: dict, dry_run: bool = False) -> int | None:
        ...
        for lbl in labels:
            try:
                gh("label", "create", lbl, "--repo", REPO,
                   "--color", "0E8A16" if lbl == "klickdummy" else "C5DEF5",
                   check=False)
            except Exception:
                pass
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Klickdummy → GitHub Issue + Project sync")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--only", type=str, default=None,
                        help="Nur diesen KD-Namen syncen")
